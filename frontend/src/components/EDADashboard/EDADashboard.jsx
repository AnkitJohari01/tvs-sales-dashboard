...
const GOLD   = '#E2B04A';
const CARD   = '#16213E';
const DONUT_COLORS = ['#E2B04A', '#F4C87A', '#C89A3A', '#A07820', '#7A5C10', '#94A3B8'];
        ...
        <div className="eda-grid animate-fade-in-up" style={{animationDelay:'160ms'}}>
            ...
            <div className="eda-chart-wrapper">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={d.branch_sales} margin={{top:20,right:20,left:10,bottom:60}}>
                  <CartesianGrid strokeDasharray="3 3" stroke="rgba(78,204,163,0.1)" vertical={false} />
                  <XAxis dataKey="Branch" stroke="#94A3B8" fontSize={10} angle={-35} textAnchor="end" interval={0} />
                  <YAxis tickFormatter={fmtINR} stroke="#94A3B8" fontSize={11} />
                  <Tooltip content={<DarkTooltip />} />
                  <Bar dataKey="HistRevenue" fill={NAVY} radius={[4,4,0,0]} name="Sales">
                    <LabelList dataKey="HistRevenue" position="top" formatter={fmtINR} fill="#CBD5E1" fontSize={10} />
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="eda-chart-wrapper">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={d.daily_trend} margin={{top:10,right:20,left:10,bottom:5}}>
                  <CartesianGrid strokeDasharray="3 3" stroke="rgba(78,204,163,0.1)" />
                  <XAxis dataKey="date" tickFormatter={fmtDate} stroke="#94A3B8" fontSize={10} interval="preserveStartEnd" />
                  <YAxis tickFormatter={fmtINR} stroke="#94A3B8" fontSize={11} />
                  ...
                  {bestDay && <ReferenceDot x={bestDay.date} y={bestDay.sales} r={6} fill={TEAL} stroke="#fff" label={{value:`Best: ${fmtINR(bestDay.sales)}`, fill:'#4ECCA3', fontSize:10, position:'top'}} />}
                  {worstDay && <ReferenceDot x={worstDay.date} y={worstDay.sales} r={6} fill="#EF4444" stroke="#fff" label={{value:`Low: ${fmtINR(worstDay.sales)}`, fill:'#EF4444', fontSize:10, position:'bottom'}} />}
                </LineChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="eda-chart-wrapper eda-chart-wrapper--donut">
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Legend verticalAlign="bottom" wrapperStyle={{fontSize:'11px',color:'#CBD5E1'}} />
                </PieChart>
              </ResponsiveContainer>
            </div>
          ...
          <div className="eda-card">
            ...
            <div className="eda-chart-wrapper">
              {d.forecast_vs_actual?.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={d.forecast_vs_actual} margin={{top:10,right:20,left:10,bottom:5}}>
                    <CartesianGrid strokeDasharray="3 3" stroke="rgba(78,204,163,0.1)" />
                    <XAxis dataKey="ForecastedDate" tickFormatter={fmtDate} stroke="#94A3B8" fontSize={10} interval="preserveStartEnd" />
                    <YAxis tickFormatter={fmtINR} stroke="#94A3B8" fontSize={11} />
                    <Tooltip content={<DarkTooltip labelFormatter={fmtDate} />} />
              ...
              ) : <p style={{color:'#94A3B8',textAlign:'center',paddingTop:'4rem'}}>No forecast data available.</p>}
            </div>
          </div>
...
function CalendarHeatmap({ data, month }) {
  if (!data?.length) return <p style={{color:'#94A3B8'}}>No calendar data.</p>;
  ...
  const getColor = (sales) => {
    ...
    const r = Math.round(255 - ratio * (255 - 26));
    const g = Math.round(248 - ratio * (248 - 26));
    const b = Math.round(231 - ratio * (231 - 46));
    return `rgb(${r},${g},${b})`;
  };
